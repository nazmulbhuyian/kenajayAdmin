...
const AttributePage = () => {
  ...
  const {
    ...
    queryKey: [
      `/api/v1/attribute/dashboard?page=${page}&limit=${limit}&searchTerm=${searchTerm}&role_type=attribute_show`,
    ],
    queryFn: async () => {
      try {
        const res = await fetch(
          `${BASE_URL}/attribute/dashboard?page=${page}&limit=${limit}&searchTerm=${searchTerm}&role_type=attribute_show`,
          {
            credentials: 'include',
          }
        )
